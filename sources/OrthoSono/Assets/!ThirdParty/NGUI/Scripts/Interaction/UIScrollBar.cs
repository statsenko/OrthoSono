...
public class UIScrollBar : MonoBehaviour
{
	...
	const float minScrollBarColliderWidth = 40f;
	/// <summary>
			...
			if (mDir != value)
			{
				...
				if (mBG != null)
				{
						...
						(mDir == Direction.Horizontal && scale.x < scale.y))
					{
						...
						{
							UpdateCollider(mBG.gameObject);
						}
						if (mFG.collider != null) 
						{
							UpdateCollider(mFG.gameObject);
						}
					}
				}
			}
	...
	void UpdateCollider(GameObject go_)
	{
		BoxCollider boxCollider_ = NGUITools.AddWidgetCollider(go_);

		Vector3 colliderSize_ = boxCollider_.size;

		if (colliderSize_.x*go_.transform.localScale.x < minScrollBarColliderWidth)
			boxCollider_.size = new Vector3(minScrollBarColliderWidth/go_.transform.localScale.x, colliderSize_.y, colliderSize_.z);
	}
