		...
		if (label != null && enabled && NGUITools.GetActive(gameObject))
		{
			if (isSelected)
			{
					...
					)
				{
					...
					mKeyboard = CustomKeyboard.Open(mText, (TouchScreenKeyboardType)((int)type), isPassword?false:autoCorrect, false, isPassword);
	...
	void UpdateLabel ()
	{
		...
		if (label.font != null)
		{
			...
			if (!label.shrinkToFit)
			{
				if (label.multiLine)
				{
					processed = label.font.WrapText(processed, label.lineWidth / label.cachedTransform.localScale.x, 0, false, UIFont.SymbolStyle.None, (isPassword)?"...":null);
				}
				else
				{
					...
					if (fit != processed)
					{
						...
						if (mPivot == UIWidget.Pivot.Center || mPivot == UIWidget.Pivot.Bottom || mPivot == UIWidget.Pivot.Top)
						{
							pos.x = mPosition + label.lineWidth/2f;
