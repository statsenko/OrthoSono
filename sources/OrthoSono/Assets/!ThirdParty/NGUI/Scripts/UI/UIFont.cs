...
public class UIFont : MonoBehaviour
{
	...
	public string WrapText (string text, float maxWidth, int maxLineCount, bool encoding, SymbolStyle symbolStyle, string addWhenNotFit = null)
	{
		...
		for (; offset < textLength; ++offset)
		{
			...
			if (remainingWidth < 0)
			{
				...
				if (lineIsEmpty || !multiline || lineCount == maxLineCount)
				{
					...
					{
						if (!string.IsNullOrEmpty( addWhenNotFit )) 
							sb.Append(addWhenNotFit);
						start = offset;
						break;
					}
					EndLine(ref sb);
