...
public class UICamera : MonoBehaviour
{
	...
	static public UICamera eventHandler
	{
		get
		{
			for (int i = 0; i < mList.Count; ++i)
			{
				...
				if (cam == null || !cam.enabled || !NGUITools.GetActive(cam.gameObject) || LoadingOrAlertDisablingEvents(cam)) continue;
				return cam;
			}
			return null;
		}
	...
	static public bool Raycast (Vector3 inPos, ref RaycastHit hit)
	{
		for (int i = 0; i < mList.Count; ++i)
		{
			...
			if (!cam.enabled || !NGUITools.GetActive(cam.gameObject) || LoadingOrAlertDisablingEvents(cam)) continue;
