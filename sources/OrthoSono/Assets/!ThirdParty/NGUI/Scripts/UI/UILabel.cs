...
public class UILabel : UIWidget
{
	...
	public override Vector2 relativeSize
	{
		get
		{
			...
			if (hasChanged) ProcessText();
			//Now method returns size for visible text
			Vector2 size_ = !string.IsNullOrEmpty(mProcessedText) ? mFont.CalculatePrintedSize(mProcessedText, mEncoding, mSymbols) : Vector2.one;
			return size_;
		}
	}
...
//We need to override OnDrawGizmos for UILabel components, because now it's shouldn't use relativeSize property 
#if UNITY_EDITOR
	void OnDrawGizmos ()
	{
		#if OLD_UNITY
