...
public class UISprite : UIWidget
{
	...
	public override Vector4 border
	{
		get
		{
			if (type == Type.Sliced || type == Type.SlicedTiled)
			{
	...
	public virtual Vector2 inner
	{
		get 
		{
			...
			return new Vector2(inner.width, inner.height)* atlas.pixelSize;
		}
	}
	...
	public override bool pixelPerfectAfterResize { get { return (type == Type.Sliced || type == Type.SlicedTiled); } }
	...
	{
		if ((type == Type.Sliced || type == Type.Tiled || type == Type.SlicedTiled) && cachedTransform.localScale != mScale)
		{
	...
	public override void MakePixelPerfect ()
	{
		...
		else if (t == Type.Tiled || t == Type.SlicedTiled)
		{
	...
	public override void OnFill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		switch (type)
		{
			...
			case Type.SlicedTiled:
			SlicedTiledFill(verts, uvs, cols);
			break;
...
#region Various fill functions
	...
	protected void SlicedTiledFill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		float wc = Mathf.Abs(mInner.width/scale.x) * pixelSize;
		float wl =  Mathf.Abs(v[1].x - v[0].x);
		float wr =  Mathf.Abs(v[3].x - v[2].x);
		
		float hc = Mathf.Abs(mInner.height/scale.y) * pixelSize;
		float ht = Mathf.Abs(v[1].y - v[0].y);
		float hb = Mathf.Abs(v[3].y - v[2].y);
		
		// Safety check. Useful so Unity doesn't run out of memory if the sprites are too small.
		if (wc < 0.01f || hc < 0.01f)
		{
			Debug.LogWarning("The sliced tiled sprite (" + NGUITools.GetHierarchy(gameObject) + ") is too small.\nConsider using a bigger one.");
			Debug.LogWarning(string.Format("Center Size: {0} {1}", wc, hc));
			wc = hc = 0.01f;
		}
		...
		for (int x = 0; x < 3; ++x)
		{
			...
			for (int y = 0; y < 3; ++y)
			{
				...
				Vector2 minUV_ = new Vector2(uv[x].x, uv[y].y);
				Vector2 maxUV_ = new Vector2(uv[x2].x, uv[y2].y);
				Vector2 clippedUV_ = maxUV_;
				...
				while (y_ > v[y2].y)
				{
					...
					float x_ = v[x].x;
					float y2_ = y_ - height_;
					clippedUV_.x = maxUV_.x;

					if (y2_ < v[y2].y)
					{
						clippedUV_.y = minUV_.y + (maxUV_.y - minUV_.y) * (v[y2].y - y_) / (y2_ - y_);
						y2_ = v[y2].y;
					}
					...
					while (x_ < v[x2].x)
					{
						...
						if (x2_ > v[x2].x)
						{
							clippedUV_.x = minUV_.x + (maxUV_.x - minUV_.x) * (v[x2].x - x_) / (x2_ - x_);
							x2_ = v[x2].x;
						}
					
						verts.Add(new Vector3(x2_, y_, 0f));
						verts.Add(new Vector3(x2_, y2_, 0f));
						verts.Add(new Vector3(x_, y2_, 0f));
						verts.Add(new Vector3(x_, y_, 0f));

						uvs.Add(new Vector2(clippedUV_.x, minUV_.y));
						uvs.Add(new Vector2(clippedUV_.x, clippedUV_.y));
						uvs.Add(new Vector2(minUV_.x, clippedUV_.y));
						uvs.Add(new Vector2(minUV_.x, minUV_.y));
