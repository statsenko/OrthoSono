...
static public class NGUITools
{
	...
	static public BoxCollider AddWidgetCollider (GameObject go, bool includeInactive = false)
	{
		if (go != null)
		{
			...
			int depth = NGUITools.CalculateNextDepth(go, includeInactive);

			Bounds b = includeInactive ?NGUIMath.CalculateRelativeWidgetBoundsIncludeInactive(go.transform, go.transform) :NGUIMath.CalculateRelativeWidgetBounds(go.transform);
			box.isTrigger = true;
	...
	static public int CalculateNextDepth (GameObject go, bool includeInactive = false)
	{
		int depth = -1;
		UIWidget[] widgets = go.GetComponentsInChildren<UIWidget>(includeInactive);
	...
	static public T FindInChilds<T> (GameObject go) where T : Component
	{
		if (go == null) return null;
		...
		if (comp == null)
		{
			...
			{
				comp = FindInChilds<T>(t.gameObject);
				if (comp != null)
					break;
			}
		}
	...
	static public void MarkParentAsChanged (GameObject go, bool checkMaterial = true)
	{
		UIWidget[] widgets = go.GetComponentsInChildren<UIWidget>();
		for (int i = 0, imax = widgets.Length; i < imax; ++i)
			widgets[i].ParentHasChanged(checkMaterial);
	}
